...
import { collection, getFirestore, doc, getDoc, updateDoc, getDocs } from "firebase/firestore";
  ...
  methods: {
    ...
    async deleteCard(cardName, index) {
      try {
        ...
        if (docSnap.exists() && docSnap.data().Inventory) {
          // const updatedInventory = docSnap.data()
          //   .Inventory.filter((card) => card.name !== cardName);
