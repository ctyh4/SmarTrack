  ...
  <div v-if="user">
    <div class="sidebar-container">
      <div id="mySidenav" class="sidenav" :class="{ openNavClass: isActive }">
        ...
        <a class="page-router" @click="$router.push('/Payment')"
          >Payment Recommendation</a
        >
        <a class="page-router" @click="$router.push('/Tracking')"
          >Tracking & Analysis</a
        >
      </div>

      <div id="main" :class="{ pushMainContent: isActive }">
        <span style="font-size: 30px; cursor: pointer" @click="toggleNav"
          >&#9776;</span
        >
        <span
          style="
...
#main {
  ...
  transition: margin-left 0.5s; /* Smooth transition for the main content */
  padding: 16px;
  width: 100%;
